...
class Environment(object):
    ...
    class Log():
            ...
            def header(self, cmd, msg, options=None, fnl=True, enl=True):
                """Log method to print command title and command environment to the shell"""
                
                # Local function variables:
                local_msg_length = len(msg)
                ...
                click.secho("{}".format("=" * local_msg_length), fg='yellow')
                # Print environment metadata info
                self.info("{}:{}: Invoking command: magicdoc tf {}".format(LOG_METADATA, cmd, cmd))
                self.info("{}:{}: Instantiating `magicdoc tf {}` environment...".format(LOG_METADATA, cmd, cmd))
                # Print command argument info
                if options is None:
                    self.info("{}:{}: No arguments required.".format(LOG_METADATA, cmd)
                else:
                    if isinstance(options, dict) and bool(options):
                        for k, v in options.items():
                            self.args("{}:{}: {}".format(LOG_METADATA, cmd, k), v, enl=False)
                if enl:
                    click.echo()
                # Return the value for functions this variable.
                return "{}:{}".format(LOG_METADATA, cmd)

            def args(self, msg, option, fnl=False, enl=True):
...
def cli(ctx, verbose: bool, verbose_level: str, directory: str, exclude_dir: str, config: str, no_recursion: bool):
    ...
    # Assign function identifier, log and declare the tf environment.
    this = "MagicDocCMD:tf"
...
def env(ctx):
    ...
    this, log = ctx.set_context('env', 'Environment Configuration', {})
    log = ctx.obj.log
    
    # HEADER Command function header.
    log.header("MagicDoc Terraform Environment Configuration:")
    log.info("Working with returned file object:")
        log.debug(json.dumps(files, indent=4, sort_keys=True))
    
    log.info("Invoking command magicdoc tf env")
    log.info("{}: Instantiating `magicdoc tf env` environment...".format(this))
    log.info("{}: No arguments required.".format(this) enl=True)

    # ENV_VARIABLES: Define any local environments that the command function requires.
    local_verbose = "Enabled" if ctx.obj.verbose else "Disabled"
    local_recursion = "Disabled" if ctx.obj.no_recursion else "Enabled"
    local_project_config = "Found" if bool(ctx.obj.project_config) else "Not Found"
    local_tf = "Found" if bool(ctx.obj.tf) else "Not Found"
    log.info("{}: Setting local function values...".format(this))
    log.args("{}: Verbose".format(this), local_verbose, enl=False)
    log.args("{}: Directory Recursion".format(this), local_recursion, enl=False)
    log.args("{}: Project Config".format(this, local_project_config, enl=False))
    log.args("{}: Terraform Project Files".format(this, local_tf))
